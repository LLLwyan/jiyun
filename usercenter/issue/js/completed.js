...
function queryDetailsSuccess(data){
	...
	BaseForeach(data.data,function(i,item){
		...
		details +='<td align="left">'+item.mobile+'</td></tr>';
		details +='<tr><th width="150"  align="right">工单标题：</th>';
		details +='<td align="left">'+item.businessName+'</td>';
		details +='<th width="150"  align="right">工单类型：</th>';
		...
		details +='<td align="left">'+item.issueName+temp+'</td></tr>';
		...
		if(item.attachmentUrl !=""){
			...
			if(type.toLowerCase() == 'jpg' || type.toLowerCase() == 'gif'|| type.toLowerCase() == 'png'||type.toLowerCase() == 'jpeg'||type.toLowerCase() == 'bmp'){
				details+='<tr><th width="150" align="right">文件详情:</th><td align="left" colspan="3" class="example"><a href="#mengban_'+i+'"><img id="SmallImglogo_'+i+'"  src="'+item.attachmentUrl+'" width="50" height="50" class="img-rounded" onclick="showDiv('+i+')" style="display:block; cursor:pointer" /></a></td></tr>';
				showpic='<div id="mengban_'+i+'" class="mengban" style="display: none;"></div> <div id="tupian_'+i+'" class="tupian" style="display: none;"><a href="#" onclick="closeDiv('+i+')" style="cursor:pointer;text-decoration: none;"><img src="'+item.attachmentUrl+'" id="ii_'+i+'" /></a></div>';
			}else{
				index =  item.attachmentUrl.lastIndexOf('/');
				type = item.attachmentUrl.substring(index+1,item.attachmentUrl.length);
				details+='<tr><th width="150" align="right">文件下载:</th><td align="left" colspan="3"><a class="manager-btn mr-10" href="'+item.attachmentUrl+'" >下载</a></td></tr>'
			}
		}
	});
	if(state==3){
...
function queryContent(){
	...
	service = Commonjs.jsonToString(service)
	var params = Commonjs.getParams(fn,service);//获取参数
	Commonjs.ajaxTrue(weburl,params,queryContentSuccess,false);
}
...
function queryContentSuccess(data){
	...
	if(isNotNull(data.data)){
		...
		BaseForeach(list,function(i,item){
			...
			if(item.attachmentUrl !=""){ 
				...
				var type = item.attachmentUrl.substring(index+1,item.attachmentUrl.length);
				if(type.toLowerCase() == 'jpg' || type.toLowerCase() == 'gif'|| type.toLowerCase() == 'png'||type.toLowerCase() == 'jpeg'||type.toLowerCase() == 'bmp'){
					content+='<a href="#mengban_'+i+'" style="margin-top:10px; display:block; cursor:pointer"><img id="SmallImglogo_'+i+'"  src="'+item.attachmentUrl+'" style="height:120px;width:120px;" onclick="showDiv('+i+')"  /></a>';
					showpic2+='<div id="mengban_'+i+'" class="mengban2" style="display: none;"></div> <div id="tupian_'+i+'" class="tupian2" style="display: none;"><a href="#" onclick="closeDiv('+i+')" style="cursor:pointer;text-decoration: none;"><img src="'+item.attachmentUrl+'" id="ii_'+i+'" /></a></div>';	
				}else{ 
					index =  item.attachmentUrl.lastIndexOf('/');
					type = item.attachmentUrl.substring(index+1,item.attachmentUrl.length);
					content+='<div style="margin-top:10px;">文件下载：<a class="manager-btn  mr-10"  href="'+item.attachmentUrl+'" >下载</a></div>';
				}
			}
	...
	$('body').append(showpic2);
	
	var time=data.data.time;
	if(time!=null)
		$("#closeTime").html(data.data.time);
}
...
function replyIssue(){
	...
	service = Commonjs.jsonToString(service)
	var params = Commonjs.getParams(fn,service);//获取参数
	Commonjs.ajaxTrue(weburl,params,replyIssueSuccess,true,"正在提交...");
}
