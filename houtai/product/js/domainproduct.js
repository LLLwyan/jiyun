...
//获取域名类型
function queryDomainType(){
	var service = {};
	service.paramEName = "domainType";
	...
	if(data.result == "success"){
		...
		$('#domaintype').html(html);	
	}
}

//获取域名后缀
function queryDomainSuffix(){
	var service = {};
	var fn="querDicAllSuffix";
	service = Commonjs.jsonToString(service);
	...
	if(data.result == "success"){
		...
		if (data.data.length>0){
			BaseForeach(data.data,function(i,item){
				var idstr=item.name.replace('.', '');
				idstr=idstr.replace('.', '');
				html+='<input type="checkbox" class="input_chk" name="suffix" id="suffix_'+idstr+'" value="'+item.name+'"/>'+item.name+'&nbsp; &nbsp;&nbsp; &nbsp;';
				if(i%10==0 && i!=0){
					html+='<br>'
				}
			});
		}
		$('#domainsuffix').html(html);	
	}
}
