...
$(function(){
	...
	if(domainName==null || domainName =='' || domainName=="undefined"){
		Commonjs.alerturl("域名不能为空","./domainbusiness.html");
	}
...
function uptdns(){
	if(Commonjs.isEmpty($("#DNS1").val())){
		Commonjs.alert("请输入DNS1");
		$("#DNS1").focus();
		return false;
	}
	
	if(Commonjs.isEmpty($("#DNS2").val())){
		Commonjs.alert("请输入DNS2");
		$("#DNS2").focus();
		return false;
	}
	...
	var fn="adminUptDomainDns";
	service = Commonjs.jsonToString(service)
	var params = Commonjs.getParams(fn,service);//获取参数
	Commonjs.ajaxTrue(weburl,params,uptdnsSuccess,false);
}
...
//返回
function resReturn(){
	window.location.href= './domainbusiness.html';
}