...
function querySuccess(data){
	...
	if(view!=null && info!=null){
		...
		if (info.vhostType.indexOf('mssql') > -1) {
            ...
            if (info.sysProductView.productParam.MSSQLOnlineUrl && info.sysProductView.productParam.MSSQLOnlineUrl != '') {
                html += '&nbsp;&nbsp;<span id="mssqlOnline" style="cursor: pointer"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-preferences"></use></svg>在线管理</span>';
            }
            html += '</td>';
            html += '</tr>';
            $('#mssqlInfo').html(html);
            ...
            //在线管理.
            $('#mssqlOnline').on('click', function () {
                vmVHostDetail.mssqlOnlineEnter(info);
            });
		}
        ...
        if (info.vhostType.indexOf('mysql') > -1) {
            ...
            if (info.sysProductView.productParam.MySQLOnlineUrl && info.sysProductView.productParam.MySQLOnlineUrl != '') {
                html += '&nbsp;&nbsp;<span id="mysqlOnline" style="cursor: pointer"><svg class="icon" aria-hidden="true"><use xlink:href="#icon-preferences"></use></svg>在线管理</span>';
            }
            html += '</td>';
            html += '</tr>';
            $('#mysqlInfo').html(html);
            ...
            //在线管理
            $('#mysqlOnline').on('click', function () {
                vmVHostDetail.mysqlOnlineEnter(info);
            });
        }
...
function modFtpPassFunc(info) {
    ...
    modalWin.find('.btn-primary').unbind('click').on('click', function () {
        ...
        var service = {};
        service.id = instanceId;
        service.oldpwd = cloudEncrypt.encodePublic(oldPwd);
        service.newpwd = cloudEncrypt.encodePublic(newPwd);
        var fn="modFtpPass";
        service = Commonjs.jsonToString(service)
        var params = Commonjs.getParams(fn,service);
        parent.Commonjs.ajaxTrue(weburl,params,function () {
...
function getPwd(type) {
    ...
    modalWin.find('.btn-primary').unbind('click').on('click', function () {
        ...
        parent.Commonjs.ajaxTrue(weburl,params,function (data) {
            if (data.data.pwd) {
                modalWin.find("#findBefore").css('display', 'none');
                modalWin.find("#findAfter").css('display', '');
                modalWin.find("#findAfter").find('.pwdTxt').html(cloudEncrypt.decodePublic(data.data.pwd));
            } else {
...
var vmVHostDetail = new Vue({
    ...
    data: function () {
        return {
            ...
            mysqlOnlineFlag: false,
            mssqlOnlineFlag: false,
    ...
    methods: {
         ...
         */
        loadConfig: function() {
            var templateDir = Commonjs.getCfgVal(configParam.common.cfgKey.template);
            this.controlPanelUrl = Commonjs.getHostUrl() + templateDir + configParam.controlPanel.name;
         ...
         * 保存管理密码.
         */
        saveManagePass: function () {
            var self = this;
            this.$refs['formManagePwd'].validate( function (valid) {
                if (valid) {
                    if (self.formMP.newPwd != self.formMP.surePwd) {
                        self.$message.error('两次输入的密码不一致');
                        return false;
                    }

                    var service = {};
                    service.webId = instanceId;
                    service.newPwd = self.formMP.newPwd;
                    var fn="vHostSetManagePwd";
                    service = JSON.stringify(service);
                    var params = Commonjs.getParams(fn,service);
                    self.dialogFormVisible = false;
                    self.$refs['formManagePwd'].resetFields();
                    parent.Commonjs.ajaxTrue(weburl,params,function () {
         ...
         * @param info
         */
        mysqlOnlineEnter: function (info) {
            var html = '<form name="form1" method="post" action="' + info.sysProductView.productParam.MySQLOnlineUrl + '" ID="Form1" target="_blank">\n' +
                '\t<input name="mysqlhost" type="hidden" value="' + info.extendInfo.openMysqlIp + '">\n' +
                '\t<input name="mysqlport" type="hidden" value="3306">\n' +
                '\t<input name="pma_username" type="hidden" value="' + info.extendInfo.MySQLUserName + '">\n' +
                '\t<input name="pma_password" type="hidden" value="' + cloudEncrypt.decodePublic(info.extendInfo.mysqlOnlinePass) + '">\n' +
                ...
                '\t<input type="submit" name="Submit" id="mySqlForm" style="display: none" value="login...........">\n' +
                '\t</form>\n';
            $('#_mysqlLoginDiv').html(html);
            $('#mySqlForm').trigger('click');
         ...
         * @param info
         */
        mssqlOnlineEnter: function (info) {
            var url = info.sysProductView.productParam.MSSQLOnlineUrl + '?server=' + info.extendInfo.openMssqlIp +
                "&username=" + info.extendInfo.DbUserName + "&password=" + cloudEncrypt.decodePublic(info.extendInfo.mssqlOnlinePass);
            window.open(url);
        },
        /**
         * 退出登录.
         */
        logoutControlPannel: function () {
            parent.ControlPanelPublic.logout();
        }
    },
    mounted: function () {
        this.loadConfig();
