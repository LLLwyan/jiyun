...
function getHelpInfoSuccess(data){
	...
	if (data.data.length>0){
		BaseForeach(data.data,function(i,item){
			if(i%2==0){
				notice+='<li><span>'+jsonDateFormat(item.publishTime)+'</span>&nbsp;<div>&nbsp;&nbsp;<a target="_blank" href="info.html?id='+item.id+'">'+item.title+'</a></div></li>';
			}else{
				notice+='<li class="bgcolor" ><span>'+jsonDateFormat(item.publishTime)+'</span>&nbsp;<div>&nbsp;&nbsp;<a target="_blank" href="info.html?id='+item.id+'">'+item.title+'</a></div></li>';
			}
		});
